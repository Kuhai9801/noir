...
impl TaintedDescendants {
    ...
    fn try_constrain(
        ...
        self.single_outputs.retain(|output| {
            ...
            if constrained {
                if all_constrained.insert(*output) {
    ...
    fn arguments_intersect(
        ...
        for &cv in constrained_values {
            ...
            let scan = scan_tainted_ancestors(
                cv,
                &self.arg_ancestors,
...
impl Context {
    ...
    fn constrain_tainted(
        ...
        while let Some(constraint_index) = worklist.pop_front() {
            ...
            for id in active_tainted[..constraint.active_tainted_len].iter() {
...
fn scan_tainted_ancestors(
    value: ValueId,
    arg_ancestors: &HashSet<ValueId>,
    is_current_output: impl Fn(ValueId) -> bool,
    parents: &HashMap<ValueId, Vec<ValueId>>,
    equivalences: &HashMap<ValueId, Vec<ValueId>>,
    all_tainted: &ValueSet,
    all_constrained: &ValueSet,
    ...
    while let Some((current, distance, trusted_seen, blocked_by_current_output)) = queue.pop_front()
    {
        ...
        let blocked_by_current_output =
            if is_current_output(current) { !current_trusted } else { blocked_by_current_output };

        if all_tainted.contains(&current) {
            scan.blockers.push(current);
        }
        if arg_ancestors.contains(&current) && trusted_seen && !blocked_by_current_output {
            scan.accepts = true;
            return scan;
        }
        ...
        if let Some(parents) = parents.get(&current) {
            queue.extend(
                parents
                    .iter()
                    .map(|parent| (*parent, distance + 1, trusted_seen, blocked_by_current_output)),
            );
        }
